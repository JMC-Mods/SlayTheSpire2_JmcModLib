...
Third-party MODs that also intercept MOD-mismatch popups can call `OptionalNetworkMismatch.ShouldHandle(info)`. A `true` result means the error contains a JML optional-network marker and the current popup generated by JML should be preserved. When no marker is present, the caller can continue with subscription, disablement, or another ordinary mismatch workflow.
...
| `OptionalNetworkMismatch.ShouldHandle(info)` | Detects whether a mismatch should keep JML's dedicated prompt |
