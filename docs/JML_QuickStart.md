...
如果要保存的内容不是“设置页选项”，使用 `JmcModLib.Persistence`。它支持当前机器本地偏好 local preference、账号范围 global、当前 profile 范围 profile，以及当前 run 的本地非同步数据。
...
- `JmcDataSlot<T>` 用于 local/global/profile；`JmcRunDataSlot<T>` 用于当前 run。
