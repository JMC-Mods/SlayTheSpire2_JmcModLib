...
/// <summary>
/// 子 MOD 用于读写当前 run 或当前客户端本局非同步持久化数据的槽位句柄。
/// </summary>
/// <typeparam name="T">槽位保存的数据类型。</typeparam>
/// <remarks>
/// 该槽位可配合 <see cref="JmcRunDataAttribute"/> 写入本地 run save，也可配合
/// <see cref="JmcClientRunDataAttribute"/> 写入当前客户端本局 sidecar；两者都不参与多人同步。
...
public sealed class JmcRunDataSlot<T>
{
    ...
    /// <summary>
    /// 创建使用 <typeparamref name="T"/> 默认值的本局槽位。
    /// </summary>
    ...
    /// 创建带初始默认值的本局槽位。
    /// </summary>
    /// <param name="defaultValue">新 run、sidecar 或存档中没有对应数据时使用的默认值。</param>
